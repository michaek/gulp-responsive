...
module.exports = async function sharpVinyl (file, config, options) {
  ...
  const image = sharp(file.contents, { density: options.density })
  ...
  try {
    ...
    if (config.composite) {
      image.composite(config.composite)
    }
    ...
    if (config.embed) {
      const lr = (config.embed.width - width) / 2
      const tb = (config.embed.height - height) / 2
      image.extend({
        top: tb,
        bottom: tb,
        left: lr,
        right: lr,
        background: config.embed.background
      })
    }
